...
tsne_50 <- tsne_50 %>% filter(!grepl('MGS', Sub_Tissue)) %>% # remove AMD
  filter(!grepl('MGS', Sub_Tissue), Tissue != "Choroid Plexus", Sub_Tissue != 'Retina - Fetal Eye')
...
retina_plus_group <- tsne_50 %>% filter(X1 > -30, X1 < -1, X2 < 50, X2 > 10, Tissue != 'RPE')
#retina_plus_group <- bind_rows(retina_plus_group, tsne_50 %>% filter(X1 > 12))
dbscan_cluster <- dbscan(retina_plus_group %>%  select(X1, X2) ,  minPts = 3, eps = 3)
...
################
load('data/data_pull.Rdata')
tsne_50 <- tsne_50 %>%   filter(Tissue != "Choroid Plexus", Sub_Tissue != 'Retina - Fetal Eye')
dbscan_cluster <- dbscan(tsne_50 %>% select(X1, X2),  minPts = 3, eps = 2.5)
tsne_50$Cluster <- dbscan_cluster$cluster
col1 = 'X1'
col2 = 'X2'
...
retina_plus_group <- tsne_50 %>% filter(X1 > -50, X1 < -5, X2 < 50, X2 > -10, Tissue != 'RPE')
#retina_plus_group <- bind_rows(retina_plus_group, tsne_50 %>% filter(X1 > 12))
dbscan_cluster <- dbscan(retina_plus_group %>%  select(X1, X2) ,  minPts = 3, eps = 3)
...
zoom_plot <- tsne_50_prep %>% 
  ...
  custom_col + 
  scale_fill_manual(values = c('#7be276', '#7be276', '#8ec4f2', '#daa0ef')) +
  theme_bw() +
  guides(colour = guide_legend(override.aes = list(alpha = 1), ncol=2),
         shape = guide_legend(ncol =2 )) +
  theme(axis.text.x = element_text(size = 6),
...
svg('figures_and_tables/zoom_plot_supplemental.svg', width = 4, height = 4, family = 'sans')
zoom_plot
dev.off()
