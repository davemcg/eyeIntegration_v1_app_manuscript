...
That said ** TRY THE COOK'S DISTANCE SUGGESTION **
```
...
group <- metadata %>% 
  ...
  unique() %>% 
  pull(Tissue)
d <- DGEList(tpm[,2:ncol(tpm)], group = groups)
d <- calcNormFactors(d)
d <- estimateCommonDisp(d)
power <- c()
for (i in seq(5,100,5)){
  power <- c(power, check.power(nGenes = 37000, pi0 = 0.6, i, d$counts[,1], d$tagwise.dispersion, 2, up = 0.5,  replace = TRUE, fdr = 0.05, sims = 2)$pow_qvalue_ave)}

data %>% ggplot(aes(x=V1, y=power)) + geom_line() + theme_minimal() + labs(x='Number of Samples in Each Group', y = 'Power to Detect 1 log2(FC)') + scale_x_continuous(breaks = seq(10,100,10), labels = seq(10,100,10))
