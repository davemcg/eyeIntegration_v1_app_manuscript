...
Reviewer 2 asked why we had not included the Ratnapriya et al. large AMD retina RNA-seq study. The reason was because the data was made available after our last data pull. While addressing the reviewer criticisms we decided to pull new GTEx samples along with all eye tissue datasets as of April 2019. This will change all sample counts in the document. We **have** corrected the incorrect code that generated the wrong sample numbers.
...
Supplemental Table 2 is unnecessarily confusing. There are no differences between the "gene type" whether at the gene or transcript level. We have added the Gencode description of the biotypes to the table (Supplementary Table 2) and given the counts as "Gene Counts" or "Transcript Counts" for each biotype. 
...
We originally had 63 tissues - several were removed as they had an insufficient number of samples after quality control. I've updated the text for this figure legend to reflect the number after removing samples for QC reasons. 
...
```
1311 went into the pipeline. 30 GTEx and 62 eye samples were removed for a variety of QC reasons (see sup table). 371 + 848 + 30 + 62 = 1311. This was made more clear in the text. ** PUT IN NEW QUOTE AFTER KNITTING WORD DOCUMENTS** 
```
...
```
The colors were poorly designed. We have updated the color scheme. 

There are 33 total clusters. We have added a supplementary table summarizing the Sub Tissue composition of each one. 

tsne_50 %>% group_by(Cluster, Sub_Tissue) %>% summarise(Count = n()) %>%  mutate(Sub_Tissue = trimws(Sub_Tissue))  %>% rename(`Sub Tissue` = Sub_Tissue)  %>% flextable::regulartable()
```
...
While  ComBat is an extremely popular and powerful tool to remove batch effects, I am hesitant to use as there is some evidence that these types of tools may overinflate confidence in group differences (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4679072/). 
...
```
Parameters selected by checking distributions and picking the parameter that removes the left tail. We have added mapping rate as a supplementary figure and edited the text in the methods accordingly. 
...
I wrote a long explanation of how what we are doing is reasonable, but then thought some more and now have come around to your position. While this t-SNE / euclidean distance cutoff approach works on a empirical level, it is a poor idea.

We have implemented the outlier-by-correlation-to-group approach you suggested (Nature Genetics 46: 430, 2014).
```
...
We have added the power calculation as supplemental figure. 

While it would be nice in *principal* to just use the entire GTEx dataset, there are very practical limiations on why this is inconvenient - the size of the full GTEx datasets, raw, is around 1 petabyte of data, which is very slow to move around and do any compute on. Also this amount of storage space brushes against the limits of our compute cluster. 

As for the Ratnapriya/Swaroop data, we mention it briefly in our introduction, but we hadn't added the data to eyeIntegration as it became available shortly after our last data pull. 

We have surveyed the SRA / GEO again and have added new studies (including the Ratnapriya/Swaroop) project data to eyeIntegration.

As the AMD data they collected are likely to be of interest, we have made an exception to our "normal tissue" policy and have added the different AMD stages as selectable tissue options. 
