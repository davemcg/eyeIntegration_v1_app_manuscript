...
While organoid and retina tissues are highly similar at a pan-transcriptome level, differential expression analysis between organoid and fetal retina reveals differences in related groups of genes. Analysis of broad trends in GO term enrichment between differentially expressed genes shows that the HOXB family and protocadherins are differentially expressed between fetal retina and organoids. We noticed that several BRN3/POU4F genes were also differentially regulated
...
As human fetal tissue is difficult to obtain and thus not very amenable for chemical or genetic modification, it is crucial for iPSC- and organoid-based in vitro developmental models to be developed. Our merging of these datasets and analysis at the transcriptome level (as compared to cross-analyzing using a limited number of known marker genes) demonstrates that these two approaches successfully recapitulate fetal tissues to a first approximation at the whole transcriptome level. This result underscores the value of these approaches in building models for human retina development.
...
gene_pool_2019 %>% tbl('all_vs_all_GO') %>% filter(Set == 'Retina_Fetal.Tissue-Retina_3D.Organoid.Stem.Cell', ONTOLOGY == 'BP') %>% as_tibble() %>% mutate(qvalue = as.numeric(qvalue)) %>% arrange(qvalue) %>% filter(ID %in% c('GO:0001503','GO:0031589','GO:0050878','GO:0061448','GO:0050678','GO:0030335','GO:0030198', 'GO:0007156', 'GO:0050769', 'GO:0030099')) %>% mutate(geneID = gsub('<br>', ' ', geneID)) %>% as_tibble() %>% regulartable() %>% 
  flextable::align(align='center') %>% 
  font(fontname = 'Linux Libertine O', part = 'all')
#Output Caption (* * for italics)
cat(paste0("\n*",GOterm_cap,"*\n"))
