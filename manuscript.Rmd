...
core_both  %>% mutate(Tissue = case_when(study_accession == 'SRP012682' ~ 'GTEx', TRUE ~ Tissue),
  ...
  xlab('') + 
  geom_label_repel(stat = 'identity', position = position_dodge(width = 0.9), size = 4, ylim =c(3, NA), show.legend = FALSEx) + 
...
flextable(core_tight_2019 %>% filter(study_accession != 'SRP012682') %>% sample_n(10)) %>% 
  #left_join(mapping_rate, by = 'sample_accession') %>% 
  #        rename(mapping_rate = `Mapping Rate`)) %>% 
  font(fontname = 'Linux Libertine O', part = 'all') %>% 
  flextable::fontsize(size = 5) %>% 
  width(width = c(0.4,0.4,0.4,0.4,0.4,0.4,0.4,1,3,0.4,0.4)) %>% 
