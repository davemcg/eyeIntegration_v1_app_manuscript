...
tsne_zoom <- tsne_50_prep  %>% mutate(Group = case_when(study_accession == 'SRP012682' ~ 'GTEx',
                                                        TRUE ~ Tissue),
                                      Label = case_when(grepl('^0', Label) ~ '', TRUE ~ Label),
                                      Group = factor(Group, levels = c('GTEx', 'Cornea', 'ESC', 'Lens', 'Retina', 'RPE'))) %>% 
  filter(Group == 'Retina') %>%
...
tsne_50_prep %>% mutate(Group = case_when(study_accession == 'SRP012682' ~ 'GTEx',
  ...
  xlab('t-SNE 1') + ylab('t-SNE 2') +
  facet_zoom(xy = Group == 'Retina', zoom.size = 1, zoom.data=zoom) +
  geom_label_repel(data=tsne_zoom,size = 7, aes(label=Label), alpha=0.7, box.padding = unit(0.3, "lines")) + 
  scale_color_discrete_sequential(palette = 'viridis') + 
  guides(colour = guide_legend(override.aes = list(alpha = 1))) +
