...
```{r tsneFetalRetina, fig.height = 4, fig.cap=tsneFetalRetina_cap, echo=F}
...
retina_plus_group <- tsne_50 %>% filter(X1 > 34, X1 < 51, X2 > -25, X2 < 21)
dbscan_cluster <- dbscan(retina_plus_group %>%  select(X1, X2) ,  minPts = 3, eps = 2.0)
...
# samples closest to center in each cluster
center_samples <- retina_plus_group %>% left_join(.,core_tight_2019)  %>%
  left_join(.,cluster_centers, by=c('Cluster')) %>%
  mutate(Distance = (!!sym(col1)-C1)+(!!sym(col2)-C2)) %>%
  group_by(Cluster) %>%
  dplyr::slice(which.min(Distance)) %>%
...
# cluster stats
cluster_stats <- retina_plus_group %>% left_join(.,core_tight_2019)  %>%
  mutate(Origin=factor(Origin, levels=c('Adult Tissue', 'Fetal Tissue', 'Stem Cell Line', 'Cell Line', 'Organoid'))) %>%
  mutate(Cluster = as.factor(Cluster)) %>%
  group_by(Cluster) %>%
  summarise(Cluster_Tissues = paste(unique(Tissue), collapse=',\n'), Cluster_Counts = paste(n(), ' samples', sep=''))
...
tsne_50_prep %>% mutate(Group = case_when(study_accession == 'SRP012682' ~ 'GTEx',
                        ...
                        Group = factor(Group, levels = c('GTEx', 'Cornea', 'ESC', 'Lens', 'Retina', 'RPE'))) %>% 
  rowwise() %>% 
  mutate(Age = str_split(sample_attribute, '_')[[1]][2]) %>% 
  ggplot(aes(x=X1,y=X2)) +
  scale_shape_manual(values=c(0:20,35:50)) +
  geom_point(size=9, alpha=0.1, aes(colour=Group)) +
  geom_point(size=1, alpha=0.4, position='jitter', aes(shape=Origin)) +
  xlab('t-SNE 1') + ylab('t-SNE 2') +
  facet_zoom(xy = Group == 'Retina', zoom.size = 1, zoom.data=zoom, horizontal = F) +
  geom_label_repel(data=tsne_zoom,size = 2, aes(label=Label), alpha=0.7, box.padding = unit(0.3, "lines")) + 
  scale_color_discrete_sequential(palette = 'viridis') + 
  guides(colour = guide_legend(override.aes = list(alpha = 1))) 
```
