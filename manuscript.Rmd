...
To evaluate this question, we look at the t-SNE plot of all tissues and focus in on the four types of retina tissues (`r fig_cap(name = 'tsneFetalRetina', display = 'cite')`). We see that there are three distinct groups (`r fig_cap(name = 'tsneFetalRetina', display = 'cite')` [35, 10], [37, 10], and [47, -5]), which correspond to iPSC and Retina Organoid, the three types of developing retina, and adult retina, respectively. Despite the technical differences 
...
tsne_50_prep %>% 
  ...
  geom_point(size=2, alpha=0.8, position='jitter', aes(shape=Origin, colour = Group)) +
  xlab('t-SNE 1') + ylab('t-SNE 2') +
  ...
  stat_chull(data=tsne_zoom %>% mutate(`Tissue Cluster` = Cluster_Tissues),aes(fill=`Tissue Cluster`), alpha = 0.3) +
  scale_color_discrete_sequential(palette = 'viridis') + 
  scale_fill_discrete_sequential(palette = 'viridis') + 
  theme_bw() +
